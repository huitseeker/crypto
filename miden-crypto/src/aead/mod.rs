...
///
/// This trait provides a generic interface for AEAD schemes that can be used in IES.
/// The key design principle is that keys are derived from shared secrets rather than
/// using fixed pre-existing keys, which ensures proper cryptographic binding.
